...
public class TaskController : MonoBehaviour
{
    ...
    [SerializeField] GameObject CirclePath;
    ...
    void Start()
    {
        ...
        taskAmplitude[0] = 360; //360
        taskAmplitude[1] = 480; //480
        taskAmplitude[2] = 700; //700
        taskAmplitude[3] = 850; //850
    ...
    void Update()
    {
        ...
        {
            //スタート地点を超えたら
            if(cursorX>=startX && cursorXprev<startX && cursorY>0 && cursorY<1000)
            {
                makePosCSV();
                canGoal = false;
                startCrossed = true;
        ...
        { 
            ...
            //中心からカーソルまでの距離を配列に保存
            cursorDistanceToCenterArray.Add(Vector2.Distance(new Vector2(cursorX,cursorY), new Vector2(0,0)));
            updatePosCSV();

            if(cursorX<=0 && cursorXprev>0)
            {
                canGoal = true;
            }

            //タスクの幅から逸脱しているかをチェック
            if(Vector2.Distance(new Vector2(cursorX,cursorY), new Vector2(0,0))>=(nowTaskAmplitude/(2*Mathf.PI)-(nowTaskWidth/2)) && Vector2.Distance(new Vector2(cursorX,cursorY), new Vector2(0,0))<=(nowTaskAmplitude/(2*Mathf.PI)+(nowTaskWidth/2)))
            {
                inPath = true;
            }
            else
            ...
            //ゴール地点を超えたら
            if(canGoal && cursorX>=startX && cursorXprev<startX && cursorY>0 && cursorY<1000)
            {
                float YDist = Mathf.Abs(cursorYprev-cursorY);
    ...
    private void taskUpdate()
    {
        ...
        //サイズ変更
        CirclePath.transform.localScale = new Vector3( (int)((nowTaskAmplitude/(2*Mathf.PI) + nowTaskWidth/2) * 2), (int)((nowTaskAmplitude/(2*Mathf.PI) + nowTaskWidth/2) * 2),1);
        InnerCircle.transform.localScale = new Vector3( (int)((nowTaskAmplitude/(2*Mathf.PI) - nowTaskWidth/2) * 2), (int)((nowTaskAmplitude/(2*Mathf.PI) - nowTaskWidth/2) * 2),1);
